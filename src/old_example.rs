...
impl MainViewState {
    fn update_text(&self, key_event: KeyEvent) -> bool {
        let c: Option<u8> = key_event.key.into();
        match c {
            Some(c) => {
                if c as u8 >= '0' as u8 && c as u8 <= '9' as u8 {
                    self.action(Action::Digit(c as char))
                } else {
                    let c = c as char;
                    match c {
                        'c' => { self.action(Action::Operator('C')) }
                        'C' => { self.action(Action::Operator('C')) }
                        '+' => { self.action(Action::Operator(c)) }
                        '-' => { self.action(Action::Operator(c)) }
                        '/' => { self.action(Action::Operator(c)) }
                        '*' => { self.action(Action::Operator(c)) }
                        '.' => { self.action(Action::Operator(c)) }
            ...
            match action {
                ...
                Action::Operator(operator) => match operator {
                    'C' => {
                        ...
                        context.widget().get_mut::<Text>().0.push('0');
                        context
                            .child_by_id("input")
                            .unwrap()
                            .get_mut::<Text>()
...
widget!(MainView<MainViewState> : KeyDownHandler {
    result: Text
});
...
impl Template for MainView {
    fn template(self, id: Entity, context: &mut BuildContext) -> Self {
        ...
        self.name("MainView").width(212.0).height(336.0).result("0").child(
            Grid::create()
